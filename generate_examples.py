...
# 各年级难度描述（按学段递进；7 年级=初一，8 年级=初二，9 年级=初三）
GRADE_LEVEL = {
    "7A": ("初一新生", "刚学英语一年左右", "初一学生已学的简单词", "不超过 12 个单词"),
    "7B": ("初一学生", "初一下学期水平", "初一学生已学的简单词", "不超过 12 个单词"),
    "8A": ("初二学生", "初二年级水平", "初二学生已学的词（比初一稍丰富）", "不超过 14 个单词"),
    "8B": ("初二学生", "初二年级水平", "初二学生已学的词（比初一稍丰富）", "不超过 14 个单词"),
    "9A": ("初三学生", "初三年级水平", "初三学生已学的词", "不超过 16 个单词"),
    "9B": ("初三学生", "初三年级水平", "初三学生已学的词", "不超过 16 个单词"),
}


def build_prompt(word, cn, examples):
    level_desc, level_detail, vocab_hint, word_limit = GRADE_LEVEL.get(GRADE, GRADE_LEVEL["7A"])
    if examples:
        lines = "\n".join(f"{i+1}. {ex}" for i, ex in enumerate(examples))
        task = f"请把下面的英语例句改写成适合{level_desc}（{level_detail}）的简单句子。"
        req = "1. 保留每条例句的核心意思，单词 \"" + word + "\" 必须出现在改写后的句子中（允许 s/es/ed/ing 变形）"
        source = "原例句:\n" + lines
    else:
        task = f"请根据单词释义，为{level_desc}（{level_detail}）造 3 条简单英语例句。"
...
要求：
{req}
2. 词汇用{vocab_hint}，句子简短（{word_limit}）
3. 每条附上自然、准确的中文翻译
